...
class Parser {
  ...
  next () {
    ...
    while (this._startDate.valueOf() <= this._endDate.valueOf()) {
      ...
      // match the month
      let result = this._expression.match(localeDate, 'M')
      if (!result) {
      ...
      result = this._expression.match(localeDate, 'D') && this._expression.match(localeDate, 'd')
      if (!result) {
      ...
      // match the hour
      result = this._expression.match(localeDate, 'H')
      if (!result) {
      ...
      // match the minute
      result = this._expression.match(localeDate, 'm')
      if (!result) {
      ...
      // match the second
      result = this._expression.match(localeDate, 's')
  ...
  getExpression () {
    return this._expression.getExpression()
  }
}
